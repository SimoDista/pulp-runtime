...
static inline int plp_dma_memcpy_3d(dma_ext_t ext, unsigned int loc, unsigned int size, unsigned int *stride, unsigned int length, unsigned int *num_reps, int ext2loc);
...
static inline unsigned int pulp_idma_memcpy_3d(unsigned int const dst_addr, unsigned int const src_addr,
unsigned int num_bytes, unsigned int *dst_stride, unsigned int *src_stride, unsigned int *num_reps) {
  DMA_WRITE(src_addr, IDMA_REG32_2D_FRONTEND_SRC_ADDR_REG_OFFSET);
  DMA_WRITE(dst_addr, IDMA_REG32_2D_FRONTEND_DST_ADDR_REG_OFFSET);
  DMA_WRITE(num_bytes, IDMA_REG32_2D_FRONTEND_NUM_BYTES_REG_OFFSET);
  ...
  DMA_WRITE(src_stride[1]-src_stride[0]*(num_reps[0]-1), IDMA_REG32_2D_FRONTEND_STRIDE_SRC1_REG_OFFSET);
  DMA_WRITE(dst_stride[1]-dst_stride[0]*(num_reps[0]-1), IDMA_REG32_2D_FRONTEND_STRIDE_DST1_REG_OFFSET);
...
static inline int plp_dma_memcpy_2d(dma_ext_t ext, unsigned int loc, unsigned int size, unsigned int stride, unsigned int length, int ext2loc) {
  if (ext2loc) {
    return pulp_idma_memcpy_2d(loc, ext, length, stride, stride, size/length);
  } else {
    return pulp_idma_memcpy_2d(ext, loc, length, stride, stride, size/length);
  }
}
...
static inline int plp_dma_memcpy_3d(dma_ext_t ext, unsigned int loc, unsigned int size, unsigned int *stride, unsigned int length, unsigned int *num_reps, int ext2loc) {
  if (ext2loc) {
    return pulp_idma_memcpy_3d(loc, ext, length, stride, stride, num_reps);
  } else {
    return pulp_idma_memcpy_3d(ext, loc, length, stride, stride, size/length);
  }
}
